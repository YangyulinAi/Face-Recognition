...
% Enumerate each sample feature vector, dimension
if nargin < 2
    bTest = 0;
end
lTargB = -1; % Scaling the target range[-1, 1]
uTargB = 1;
[m,n] = size(VecFeaMat);
SVFM = zeros(m, n); 
if bTest
    ...
    for iCol = 1:n
        ...
        if upVec(iCol) == lowVec(iCol)
            ...
            SVFM(:, iCol) = lTargB  +  ( VecFeaMat(:, iCol) - lowVec(iCol) ) / ( upVec(iCol)-lowVec(iCol) ) * (uTargB-lTargB); % Test dataset
        end
    end
end